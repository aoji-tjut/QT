	...
	, ui(new Ui::Server)
{
	...
	connect(this->tcp_server, &QTcpServer::newConnection, this, &Server::Connect);	//连接成功
	connect(this, &Server::CtrlEnter, this, &Server::on_bt_send_clicked);           //自定义信号 发送数据
...
//窗口关闭事件
void Server::closeEvent(QCloseEvent* ev)
{
...
//事件过滤 Ctrl+Enter发送
bool Server::eventFilter(QObject* obj, QEvent* ev)
{
	if(obj == ui->text_send)
	{
...
//监听
void Server::on_bt_listen_clicked()
{
...
void Server::Connect()
{
	...
	//设置tcp_socket信号与槽函数
	connect(this->tcp_socket, &QTcpSocket::readyRead, this, &Server::Receive);          //接收数据
	connect(this->tcp_socket, &QTcpSocket::disconnected, this, &Server::Disconnect);	//接收数据
	void (QTcpSocket::*signal)(QAbstractSocket::SocketError) = &QTcpSocket::error;
	void (Server::*slot)(QAbstractSocket::SocketError) = &Server::Error;
...
//发送数据
void Server::on_bt_send_clicked()
{
...
//打开文件 发送文件信息
void Server::on_bt_file_clicked()
{
	//得到文件路径
